...
#define STATUS_UPDATE_RATE 0x500
...
#else
    uint32_t insn;
    uint32_t uregs_before[18];
    uint32_t uregs_after[18];
...
void execute_insn_slave(pid_t *slave_pid_ptr, uint32_t insn, execution_result *result)
{
    ...
    // Reset all regs
    memset(regs.uregs, 0, 18 * sizeof(regs.uregs[0]));
    regs.uregs[15] = insn_loc;
    regs.uregs[16] = 0x10;  // CPSR register, user mode
    if (ptrace(PTRACE_SETREGS, slave_pid, NULL, &regs) == -1) {
        perror("setregs failed");
    }

    memcpy(result->uregs_before, regs.uregs, 18 * sizeof(regs.uregs[0]));
    ...
    memcpy(result->uregs_after, regs.uregs, 18 * sizeof(regs.uregs[0]));
